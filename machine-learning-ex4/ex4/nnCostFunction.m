...
%
% -------------- first part front Propagation for Cost Function J ---------------
a1 = [ones(m, 1) X];
z2 = a1 * Theta1';
a2 = sigmoid(z2);
...
regularize = (sum(sum(Theta1(:,2:end).^2))+ sum(sum(Theta2(:,2:end).^2))) * (lambda/(2*m));
J = J + regularize;


% --------------- second part back propagation to find Gradient Descet -------------
